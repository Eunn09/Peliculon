...
$inventario = array(); // Inicializa la variable $pelicula

if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["id_inv"])) {
    $id_inv = $_GET["id_inv"];

    // Realiza una consulta para obtener los detalles de la pelicula con el ID proporcionado
    $query = "SELECT * FROM inventario WHERE id_inv = ?";
    
    // Preparar la consulta
    $stmt = $conexion->prepare($query);
    $stmt->bind_param("i", $inventario);
    $stmt->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "UPDATE inventario SET exist_inv = ?, id_peli = ?, id_suc = ? WHERE id_inv = ?";
